...
void do_substitute(string key); // for further substitute
void alpha_arr_val(char pos, string key);
// to get the alphabet array value of each plain text element (ex:plaintext = h|alphabet array = 8
// {a=0, b=1 .....z=26})
...
void do_substitute(string key)
{
    ...
    for (int i = 0; i < strlen(p); i++)
    {
        ...
        {
            printf("%c", p[i]); // print the element as it is (such as space | , | special
                                // characters | ? | etc...)
        }
    }
